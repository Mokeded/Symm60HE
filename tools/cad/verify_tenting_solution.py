...
def main():
    ...
    assert dimensions["keyboard_suspension"] == \
        "eight equal short plate-only side pads; two per outer and centre-facing edge; no PCB gasket load"
    ...
    print(f"{len(features)} valid separate bodies; envelope "
          f"{xmax-xmin:.2f} x {ymax-ymin:.2f} x {zmax-zmin:.2f} mm")
