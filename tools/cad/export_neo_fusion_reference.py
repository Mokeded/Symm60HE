...
def main():
    ...
    # The standard FFC daughterboard and its two ribbon envelopes are excluded.
    # The rigid controller and short FFCs in the pogo kernel replace them; all
    # other PCB, plate, switch and keycap bodies retain their checked planes.
    for source in base.Objects:
        if (source.TypeId != "PartDesign::Feature" or
                source.Name in {"DaughterboardPCB", "LeftRibbonCable",
                                "RightRibbonCable"}):
            continue
    ...
    manifest = {
        ...
        "keyboard_suspension": "eight equal short side gaskets; two per outer and centre-facing plate edge",
