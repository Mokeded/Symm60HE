...
def render_shapes(shapes, output, title, subtitle, gasket=False,
    ...
    draw.text((55, 28), title, font=font(34, True), fill=TEXT)
    draw.text((57, 73), subtitle, font=font(20), fill=SUB)
    ...
    if split_labels:
        draw.rounded_rectangle((width // 2 - 180, height - 58,
                                width // 2 + 180, height - 20),
                               radius=18, fill=(26, 35, 46))
        draw.text((width // 2, height - 39),
                  "independent halves — no centre bridge", anchor="mm",
                  font=font(17, True), fill=ACCENT)
...
render_dxf(ROOT / "plate" / "Symm60HE-gasket-pads.dxf",
           OUT / "gasket-pads.png", "Neo-style plate-side Poron gasket pads",
           "Four outer + four centre-side pads · no top/bottom or PCB mounts",
           gasket=True)
...
def contact_sheet():
    files = [
        ...
        "plate-wklbs2arrows.png", "plate-universal.png", "gasket-pads.png",
    ]
...
if __name__ == "__main__":
    print("rendered five split plate pairs and one gasket-pad pattern")
