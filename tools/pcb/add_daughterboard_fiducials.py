...
trial_b2 = tuple(float(value) for value in
                 os.environ.get("SYMM60HE_FID_B2", "166.0,0.0").split(","))
POSITIONS = {
    "F": ((164.0, -3.0), (180.0, -5.0), (207.0, -4.0)),
    "B": ((173.0, 17.0), trial_b2, (209.0, 18.0)),
}
...
def main():
    ...
    for side in ("F", "B"):
        base = template(side)
        for index, (x, y) in enumerate(POSITIONS[side], 1):
            fp = deepcopy(base)
            first(fp, "at")[1:3] = [x, y]
