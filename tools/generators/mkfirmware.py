...
MUX_ORDER = ("AML1", "AML2", "AML3", "AML4", "AML5",
             "AMR1", "AMR2", "AMR3", "AMR4", "AMR5")
CHANNEL_PINS = (13, 14, 15, 12, 1, 5, 2, 4)
PROFILE_LAYOUTS = ("wkl", "wklarrows", "wklbs2", "wklbs2arrows")
PROFILE_NAMES = ("WKL / split Backspace", "Arrows / split Backspace",
                 "WKL / 2U Backspace", "Arrows / 2U Backspace")


def key_number(net):
    side, number = net.split("_")[1][0], int(net[-2:])
    return number if side == "L" else 33 + number
...
def main():
    ...
    active_key_masks = []
    default_keymaps = []
    for build in PROFILE_LAYOUTS:
        mask = [0] * 69
        for sw in physical:
            if build in sw["in_builds"].split():
                mask[sw["key"]] = 1
        active_key_masks.append(mask)
        default_keymaps.append([
            [keycode if mask[index] else "KC_NO"
             for index, keycode in enumerate(default)],
            ["_______"] * 69,
            ["_______"] * 69,
            ["_______"] * 69,
        ])
    config = {
        ...
        "keyboard": {"num_profiles": 4, "num_layers": 4, "num_keys": 69,
                     "num_advanced_keys": 32,
                     "profile_names": list(PROFILE_NAMES),
                     "active_key_masks": active_key_masks},
        "hardware": {"hse_value": 12000000, "driver": "at32f405xx"},
        "analog": {"invert_adc": False, "mux": {
            "select": ["C1", "C2", "C3"],
            "input": ["A3", "A4", "A5", "A6", "A2",
                      "A7", "C4", "C5", "B0", "A1"],
            "matrix": matrix}},
    ...
    (OUT / "README.md").write_text(
        ...
        "Generated from the PCB channel map by `tools/generators/mkfirmware.py`. Supports "
        "rapid trigger, adjustable actuation, SOCD/advanced-key bindings, four "
        "profiles, calibration, and the libhmk web configurator. The four "
        "profiles are fixed to WKL/split Backspace, Arrows/split Backspace, "
        "WKL/2U Backspace, and Arrows/2U Backspace respectively. Inactive "
        "Hall channels are masked in firmware and are not calibrated or "
        "processed.\n")
    print("matrix:", matrix)
    print("layout physical positions:", sum(map(len, layout)), "logical keys: 69")
