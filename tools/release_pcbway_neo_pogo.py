...
def main():
    cli = find_cli()
    subprocess.run([sys.executable, str(ROOT / "tools/panelize_neo_pogo.py")], check=True)
    subprocess.run([sys.executable, str(ROOT / "tools/verify_neo_pogo.py")], check=True)
    if OUT.exists():
        shutil.rmtree(OUT)
