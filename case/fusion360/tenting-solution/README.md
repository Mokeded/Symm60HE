...
- One short 12-way FFC connects the rigid controller to each floating spring
...
| Short 12-way FFC | 2 | 1.0 mm pitch; length selected from assembled CAD |
...
The keyboard itself is suspended only from eight equal short, smooth plate-side
gasket tabs: two on each outer and centre-facing plate edge. The interior pairs
use the edge sections closest to the split. There are no top/bottom gasket tabs
and no gasket mount on either Hall PCB.
